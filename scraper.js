...
export async function getFlights(from, to, date, retry = 2) {
  for (let i = 0; i <= retry; i++) {
    try {
      ...
      let dateArg = date.replace(/-/g, '').concat('0000');
      ...
      body = body.replace(/(^|&)(B_DATE_1=|WDS_DATE=)[0-9]{12}($|&)/g,
        ($0, $1, $2, $3) => $1 + $2 + dateArg + $3);

      console.log(`fetching ${from}_${to}_${dateArg}`);
      ...
      if (!response.ok) {
        fs.writeFileSync(`error_log/${from}_${to}_${dateArg}_response.json`, JSON.stringify(response));
      ...
      try {
        ...
        fs.writeFileSync(`error_log/${from}_${to}_${dateArg}_response.html`, responseText);
        throw e;
      }
